...
@media print {
  ...
  #faculty-ranking-printable,
  #faculty-ranking-printable * { visibility: visible; }
  #faculty-ranking-printable {
    position: fixed;
    left: 0; top: 0;
    width: 100%;
...
export default function FacultyRanking() {
    ...
    useEffect(() => {
        ...
        styleTag.innerHTML = PRINT_STYLE;
    ...
    const computeRankings = async () => {
        ...
        try {
            ...
            let rank = 1;
            const ranked = rows.map((row, idx) => {
                if (idx > 0 && row.totalScore < rows[idx - 1].totalScore) rank = idx + 1;
                return { ...row, rank };
            });
    ...
    const selectedSkillName = allSkills.find(s => s.id === selectedSkill)?.skill_name || "";
    const selectedCategoryName = skillCategories.find(c => c.category_id === selectedCategory)?.skill_category || "All Categories";
    ...
    return (
        <Container className="py-4">
                ...
                <Card.Body className="p-4">
                    ...
                    <Row className="g-3 align-items-end">
                        ...
                        <Col md={4} className="d-flex gap-2">
                            ...
                            >
                                {loading
                                    ? <><Spinner size="sm" animation="border" className="me-2" />Ranking…</>
                                    : "🏆 Generate Ranking"
                                }
                            </Button>
                        </Col>
            ...
            {searched && !loading && (
                ...
                <div id="faculty-ranking-printable">
                    {/* Print Header (only visible when printing) */}
                    <div className="d-none d-print-block mb-4 text-center">
                        <h4 className="fw-bold">SkillSync — Faculty Ranking Report</h4>
                        <p className="text-muted mb-0">Department: {chairInfo?.departments?.name}</p>
                        <p className="text-muted mb-0">Skill: <strong>{selectedSkillName}</strong> &nbsp;|&nbsp; Generated: {printDate}</p>
                        <hr />
                    </div>

                    {/* Result Heading + Print Button */}
                    <div className="d-flex align-items-center justify-content-between mb-3 no-print">
                        <div>
                            <h5 className="fw-bold mb-0">
                                Rankings for <span className="text-primary">{selectedSkillName}</span>
                            </h5>
                            <p className="text-muted small mb-0">
                                {rankings.length} faculty member{rankings.length !== 1 ? "s" : ""} evaluated
                            </p>
                        </div>
                        <div className="d-flex gap-2 align-items-center">
                            <Badge bg="secondary" className="px-3 py-2">
                                {rankings.filter(r => r.totalScore > 0).length} with credentials
                            </Badge>
                            {/* ── FEATURE: Print/Export Button ── */}
                            <Button
                    ...
                    {/* Print heading (screen only) */}
                    <div className="d-print-none mb-2">
                        <small className="text-muted fst-italic">
                            Tip: To export as PDF, click "Print / Export PDF" then choose "Save as PDF" in your browser's print dialog.
                        </small>
                    </div>
                                        ...
                                        <Col md={4} key={row.facultyId}>
                                            ...
                                            >
                                                <Card.Body className="p-4">
                                                    <div
                                                        className="rounded-circle d-inline-flex align-items-center justify-content-center mb-3"
                                                        style={{ width: 56, height: 56, background: bg, color, fontSize: "1.4rem", fontWeight: 700 }}
                                                    >
                                                        {label}
                                                    </div>
                                                    <h6 className="fw-bold mb-1">{row.facultyName}</h6>
                                                    <p className="text-muted small mb-2">{ordinal(row.rank)} Place</p>
                                                    <h4 className={`fw-bold text-${scoreVariant(row.totalScore)} mb-2`}>
                                                        {row.totalScore.toFixed(1)}%
                                                    </h4>
                                                    <ProgressBar
                                                        now={row.totalScore}
                    ...
                    <Card className="border-0 shadow-sm">
                            ...
                            <table className="table table-hover align-middle mb-0">
                                <thead className="table-light">
                                    <tr className="small text-uppercase fw-bold text-muted">
                                        ...
                                        <th className="text-center" style={{ width: 200 }}>Total Proficiency</th>
                                        <th className="text-center no-print" style={{ width: 130 }}>Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {rankings.length === 0 ? (
                                        <tr>
                                            <td colSpan={configs.length + 4} className="text-center py-5 text-muted">
                                                No results found.
                                            </td>
                                        </tr>
                                    ) : (
                                        rankings.map((row, idx) => {
                                            ...
                                            return (
                                                <tr
                                                    key={row.facultyId}
                                                    style={!hasCredentials ? { opacity: 0.45, background: "#fafafa" } : {}}
                                                >
                                                    <td className="ps-4">
                                                        <span
                                                            className="rounded-pill px-2 py-1 fw-bold small"
                                                            style={{ background: bg, color, minWidth: 40, display: "inline-block", textAlign: "center" }}
                                                        >
                                                            {label}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="px-3">
                                                        <div className="d-flex justify-content-between mb-1">
                                                            <span className={`fw-bold small text-${scoreVariant(row.totalScore)}`}>
                                                                {row.totalScore.toFixed(1)}%
                                                            </span>
                                                        </div>
                                                        <ProgressBar
                                                    ...
                                                    <td className="text-center no-print">
                                                        <Badge
                                                            bg={hasCredentials ? scoreVariant(row.totalScore) : "light"}
                                                            text={hasCredentials ? undefined : "secondary"}
                                                            className="px-2 py-1 small"
                                                        >
                                                            {hasCredentials ? scoreLabel(row.totalScore) : "No Credentials"}
                                                        </Badge>
                                                    </td>
                                                </tr>
                                            );
                        ...
                        <Card.Footer className="bg-white border-top py-3">
                            <div className="d-flex flex-wrap gap-4 small text-muted">
                                ...
                                <span className="ms-auto fst-italic">
                                    Dimmed rows = no credentials tagged to this skill.
                                </span>
                            </div>
                        </Card.Footer>
                    </Card>
                    ...
                    <div className="d-none d-print-block mt-4 text-center text-muted small">
                        <hr />
                        <p>This report was generated by SkillSync — A Data-Driven Faculty Profiling, Skills Gap Analysis, and Ranking System.</p>
                        <p>Information Technology Department · {chairInfo?.departments?.name} · {printDate}</p>
                    </div>
                </div>
            )}
            ...
            {!searched && (
                <Card className="border-0 shadow-sm text-center py-5">
                    <Card.Body>
                        ...
                        <p className="text-muted">
                            Choose a skill category and a specific skill above, then click{" "}
                            <strong>Generate Ranking</strong> to see faculty ranked by their computed
                            proficiency score. You can then export the results as a PDF.
                        </p>
                    </Card.Body>
                </Card>
            )}
