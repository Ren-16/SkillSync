          ...
          <Card className="p-3 shadow-sm h-100 border-0">
            ...
            <Form onSubmit={handleConfigSubmit}>
              ...
              <Form.Group className="mb-2">
                <Form.Label className="small fw-bold">Source Table</Form.Label>
                <Form.Select
                  value={formConfig.source_table}
                  onChange={(e) => setFormConfig({ ...formConfig, source_table: e.target.value })}
                  required
                  disabled={!!formConfig.category} // Automatically disabled if a category is picked
                >
                  <option value="">Select Source</option>
                  <option value="educational_attainment">Educational Attainment</option>
                  <option value="certifications">Certifications</option>
              ...
              <Button
                ...
                disabled={totalCategoryWeight >= 100}
              >
                Add Category
              </Button>
            </Form>
            ...
            <Form onSubmit={handleRuleSubmit}>
              <Form.Group className="mb-2">
                ...
                <Form.Select
                  ...
                  onChange={(e) => {
                    ...
                    let isEdu = false;
                    if (selectedConfig) {
                      if (selectedConfig.source_table === "educational_attainment") {
                        targetColumn = "degree";
                        isEdu = true;
                      } else if (selectedConfig.source_table === "certifications")
                        targetColumn = "type_of_certification";
                      else if (selectedConfig.source_table === "publications") targetColumn = "indexing";
                    }
                    setFormRule({
                      ...formRule,
              ...
              {!isEducationCategory && (
                <Form.Group className="mb-2">
                  <Form.Label className="small fw-bold">Scoring Method</Form.Label>
                  <Form.Select
                    value={formRule.rule_type}
                    onChange={(e) => setFormRule({ ...formRule, rule_type: e.target.value })}
                    required
                  >
                    <option value="exact_match">Flat Rate (Points per Match)</option>
                    <option value="per_hour">Hourly Rate (Points x Hours)</option>
              ...
              <Form.Group className="mb-2">
                ...
                <Form.Select
                  ...
                  onChange={(e) => setFormRule({ ...formRule, value: e.target.value })}
                  required
          ...
          <tbody>
            {configs.map((c) => (
              <tr key={c.id}>
                <td>{c.category}</td>
                <td>
                  <code>{c.source_table}</code>
                </td>
                <td>{c.weight}%</td>
                <td>{c.max_score}</td>
                <td>
                  <Button variant="outline-danger" size="sm" onClick={() => deleteConfig(c.id)}>
                    Delete
                  </Button>
                </td>
              </tr>
          ...
          <tbody>
            {rules.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  {r.target_column} = {r.value}
                </td>
                <td>
                  <Badge bg="secondary">{r.rule_type}</Badge>
                </td>
                <td>{r.points} pts</td>
                <td>
                  <Button variant="outline-danger" size="sm" onClick={() => deleteRule(r.id)}>
                    Delete
                  </Button>
                </td>
              </tr>
